    ...
    <>
      ...
      <section id="features" className="py-24 md:py-32 relative overflow-hidden bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          {/* Feature 3: Customer Experience - MOBILE PHONE MOCKUP FIXED FOR MOBILE */}
          <div className="relative mx-auto max-w-6xl mb-32">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              <div className="md:col-span-2">
                ...
                <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-purple-500 to-pink-600 p-6 sm:p-8 lg:p-12 shadow-2xl group hover:shadow-3xl transition-all duration-500">
                  <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff10_1px,transparent_1px),linear-gradient(to_bottom,#ffffff10_1px,transparent_1px)] bg-[size:20px_20px]"></div>
                  
                  <div className="relative z-10 flex items-center justify-center">
                    {/* Phone Mockup - MOBILE RESPONSIVE */}
                    <div className="w-64 h-[520px] sm:w-72 sm:h-[600px] bg-gray-900 rounded-[2.5rem] sm:rounded-[3rem] p-2 sm:p-2.5 shadow-2xl">
                      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-28 h-5 sm:w-32 sm:h-6 bg-gray-900 rounded-b-2xl z-20"></div>
                      
                      <div className="w-full h-full bg-white rounded-[2.25rem] sm:rounded-[2.5rem] overflow-hidden relative">
                        {/* Status Bar - RESPONSIVE */}
                        <div className="bg-white px-4 sm:px-6 py-2 sm:py-3 flex justify-between items-center text-[10px] sm:text-xs border-b border-gray-100">
                          <span className="font-semibold text-gray-900">9:41</span>
                          <div className="flex space-x-1 items-center">
                            <div className="flex space-x-0.5">
                              <div className="w-0.5 sm:w-1 h-2.5 sm:h-3 bg-gray-900 rounded-full"></div>
                              <div className="w-0.5 sm:w-1 h-2.5 sm:h-3 bg-gray-900 rounded-full"></div>
                              <div className="w-0.5 sm:w-1 h-2.5 sm:h-3 bg-gray-400 rounded-full"></div>
                            </div>
                          </div>
                        </div>

                        {/* App Content - RESPONSIVE */}
                        <div className="p-4 sm:p-5 space-y-4 sm:space-y-5 pb-20 sm:pb-24 overflow-y-auto" style={{ maxHeight: 'calc(100% - 2.5rem)' }}>
                          {/* Header - RESPONSIVE */}
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-xs sm:text-sm text-gray-500">Welcome back,</p>
                              <h5 className="text-lg sm:text-xl font-bold text-gray-900" style={{ fontFamily: 'Poppins, sans-serif' }}>Sarah!</h5>
                            </div>
                            <div className="relative">
                              <Bell className="w-5 h-5 sm:w-6 sm:h-6 text-gray-400" />
                              <div className="absolute -top-0.5 sm:-top-1 -right-0.5 sm:-right-1 w-2 sm:w-2.5 h-2 sm:h-2.5 bg-red-500 rounded-full border border-white sm:border-2"></div>
                            </div>
                          </div>

                          {/* Quick Actions - RESPONSIVE */}
                          <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
                            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl sm:rounded-2xl p-4 sm:p-5 text-white shadow-lg">
                              <Package className="w-6 h-6 sm:w-7 sm:h-7 mb-1.5 sm:mb-2" />
                              <p className="text-xs sm:text-sm font-medium">New Order</p>
                            </div>
                            <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl sm:rounded-2xl p-4 sm:p-5 text-white shadow-lg">
                              <Calendar className="w-6 h-6 sm:w-7 sm:h-7 mb-1.5 sm:mb-2" />
                              <p className="text-xs sm:text-sm font-medium">Schedule</p>
                            </div>
                          </div>

                          {/* Active Orders - RESPONSIVE */}
                          <div>
                            <h6 className="font-bold text-gray-900 mb-2.5 sm:mb-3 text-sm sm:text-base" style={{ fontFamily: 'Poppins, sans-serif' }}>Active Orders</h6>
                            {[
                              { id: "#12845", status: "Processing", color: "bg-blue-500", progress: 60 },
                              { id: "#12844", status: "Ready", color: "bg-green-500", progress: 100 },
                            ].map((order, idx) => (
                              <div key={idx} className="bg-gray-50 rounded-xl sm:rounded-2xl p-3 sm:p-4 mb-2.5 sm:mb-3 shadow-sm">
                                <div className="flex items-center justify-between mb-2.5 sm:mb-3">
                                  <span className="text-xs sm:text-sm font-semibold text-gray-900">{order.id}</span>
                                  <div className={`${order.color} text-white text-[10px] sm:text-xs px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full font-medium`}>
                                    {order.status}
                                  </div>
                                </div>
                                <div className="flex items-center space-x-1.5 sm:space-x-2">
                                  ...
                                  <span className="text-[10px] sm:text-xs text-gray-500 font-medium w-9 sm:w-10 text-right">{order.progress}%</span>
                                </div>
                              </div>
                            ))}
                          </div>

                          {/* Rewards Card - RESPONSIVE */}
                          <div className="bg-gradient-to-br from-yellow-400 to-orange-500 rounded-xl sm:rounded-2xl p-4 sm:p-5 text-white shadow-lg">
                            <div className="flex items-center justify-between">
                              <div>
                                <p className="text-xs sm:text-sm opacity-90 mb-0.5 sm:mb-1">Loyalty Points</p>
                                <p className="text-2xl sm:text-3xl font-bold">2,450</p>
                              </div>
                              <Star className="w-10 h-10 sm:w-12 sm:h-12 opacity-30" />
                            </div>
                          </div>
                        </div>

                        {/* Bottom Nav - RESPONSIVE */}
                        <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 sm:px-6 py-3 sm:py-4 rounded-b-[2.25rem] sm:rounded-b-[2.5rem]">
                          <div className="flex justify-around items-center">
                            {[
                              ...
                              <button key={idx} className="relative">
                                <item.icon className={`w-5 h-5 sm:w-6 sm:h-6 ${item.active ? 'text-blue-600' : 'text-gray-400'}`} />
                                {item.active && (
                                  <div className="absolute -bottom-3 sm:-bottom-4 left-1/2 transform -translate-x-1/2 w-1.5 h-1.5 bg-blue-600 rounded-full"></div>
                                )}
                              </button>
                            ))}
                          </div>
