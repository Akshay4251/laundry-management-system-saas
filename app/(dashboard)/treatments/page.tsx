...
import {
  ...
  useDeleteTreatment,
  useToggleTreatment,
} from '@/app/hooks/use-treatments';
import type { Treatment, CreateTreatmentInput, UpdateTreatmentInput } from '@/app/types/treatment';
...
function TreatmentFormModal({ open, onClose, treatment }: TreatmentFormModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: CreateTreatmentInput | UpdateTreatmentInput = {
      ...
      if (isEditing) {
        ...
        await createTreatment.mutateAsync(payload as CreateTreatmentInput);
      }
  ...
  const isSubmitting = createTreatment.isPending || updateTreatment.isPending;
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditing
              ...
              : 'Create a new treatment type for your services'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              placeholder="Describe what this treatment includes..."
          ...
          {/* Combo Treatment Toggle */}
          <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-100">
            <div>
              <Label className="font-medium text-blue-900">Combo Treatment</Label>
              <p className="text-sm text-blue-700">
                Is this a combination of multiple treatments? (e.g., Wash + Iron)
              </p>
            </div>
            <Switch
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border">
            <div>
              <Label className="font-medium">Active Treatment</Label>
              <p className="text-sm text-slate-500">
                Inactive treatments won't appear when creating items
              </p>
            </div>
            <Switch
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            <p className="text-sm text-amber-800">
              <strong>💡 Note:</strong> After creating a treatment, you can set prices for each
...
function DeleteTreatmentDialog({ open, onClose, treatment }: DeleteTreatmentDialogProps) {
    ...
    <AlertDialog open={open} onOpenChange={onClose}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Are you sure you want to delete <strong>"{treatment.name}"</strong>? This will also
            remove all associated pricing. This action cannot be undone.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel disabled={deleteTreatment.isPending}>Cancel</AlertDialogCancel>
          <AlertDialogAction
            onClick={handleDelete}
            disabled={deleteTreatment.isPending}
            className="bg-red-600 hover:bg-red-700"
          >
            {deleteTreatment.isPending && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
            Delete
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
...
function TreatmentCard({ treatment, index, onEdit, onDelete, onToggle }: TreatmentCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            <h3 className="font-bold text-base text-slate-900">{treatment.name}</h3>
            {treatment.isCombo && (
      ...
      {treatment.description && (
        <p className="text-sm text-slate-600 mb-4 line-clamp-2">{treatment.description}</p>
      )}
      ...
      <div className="grid grid-cols-2 gap-3 pt-4 border-t">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="font-semibold text-sm text-slate-900">{formatTurnaround(treatment.turnaroundHours)}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="font-semibold text-sm text-slate-900">{treatment.itemsCount || 0}</p>
          </div>
        </div>
      </div>
...
export default function TreatmentsPage() {
  ...
  const { treatments, stats, isLoading, isError, refetch } = useTreatments({
    search: searchQuery,
    activeOnly: showActiveOnly,
  });

  const toggleTreatment = useToggleTreatment();
    ...
    <div className="flex flex-col h-full">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="px-4 lg:px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 mb-1">Treatments</h1>
            <p className="text-sm text-slate-500">
        ...
        <AnimatePresence>
          {searchQuery && (
            ...
            >
              Found <span className="font-medium text-slate-700">{treatments.length}</span> treatment
      ...
      <div className="flex-1 overflow-auto px-4 lg:px-6 pb-6">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-sm text-slate-500 font-medium">Loading treatments...</p>
          </div>
        ) : isError ? (
          <div className="text-center py-12 bg-white rounded-2xl border border-red-200">
            <p className="text-sm text-red-600 font-medium mb-2">Failed to load treatments</p>
          ...
          >
            ...
            <h3 className="text-lg font-semibold text-slate-900 mb-2">No treatments found</h3>
              ...
              <AnimatePresence mode="popLayout">
                {treatments.map((treatment, index) => (
                  <TreatmentCard
                    ...
                    onToggle={() => toggleTreatment.mutate(treatment.id)}
                  />
                ))}
              </AnimatePresence>
            </div>
      ...
      {/* Modals */}
      <TreatmentFormModal
        open={showCreateModal || !!editingTreatment}
        onClose={() => {
